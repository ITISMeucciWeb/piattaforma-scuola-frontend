      ...
      <v-col sm="4">
        <v-responsive :max-height="$refs.viewerApp ? $refs.viewerApp.clientHeight - $refs.newFieldBtn.$el.offsetHeight: 0"
          ...
          <v-card v-for="(field, key) in fields" v-bind:key="key" class="mb-2" color="other" shaped>
            <v-card-title>
              <v-text-field ref="fieldTitle" :value="field._id" @input="updateTitle(field._id, $event)"/>
            </v-card-title>
            <v-card-subtitle>{{ field._id }}</v-card-subtitle>
            <v-card-text>
              <v-checkbox v-model="field.availableToRepresentative" class="mx-2"
                          label="Disponibile solo al cordinatore"/>
              <v-select :items="fieldTypeNames" :value="fieldTypeNames[$refs.viewer.getFieldById(field._id)[1].type]"/>
            </v-card-text>
            <v-card-actions class="flex-column mx-3 pb-5" style="gap: 10px">
              ...
              <v-btn class="mx-0" width="100%" @click="takeMeToId(field._id)">
...
export interface exportedField extends field {
  type: fieldTypes,
}
...
export default class Editor extends Vue {
  ...
  newField() {
    ...
    this.fields.push({_id: id, availableToRepresentative: false})
  ...
  moveField(index) {
    this.$refs.viewer.moveField(this.fields[index]._id, this.$refs.viewer.currentPage);
  }

  removeField(index) {
    this.$refs.viewer.removeField(this.fields[index]._id);
  ...
  getFields(): exportedField[] {
    return this.fields.map(field => {
      return {
        ...field,
        type: this.$refs.viewer.getFieldById(field._id)[1].type,
      }
    });
  }
}
